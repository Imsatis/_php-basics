<?php
function domain_blacklist_status() {
        $update_arr = array("setting_value" => date("Y-m-d"));
        $this->Mailget_Panel_model->update_admin_setting_ip_chk("ip_call_date", $update_arr);
        $dnsbl_lookup = array(
            "bl.suomispam.net",
            "dbl.suomispam.net",
            "access.redhawk.org",
        ...
        if (!empty($user_data_arr)) {
            ...
            foreach ($user_data_arr as $user_data_arr_row) {
                    ...
                    foreach($domian_lookup as $domainName) {
                        ...
                        if(!ip2long($ip)) {
                            ...
                            if (checkdnsrr($ip. '.' . $domainName . '.', "A")) {
                                ...
                                $ip_status = 'yes';
                                $txtrecords = dns_get_record($ip. '.' . $domainName . '.', DNS_TXT);

                                (!empty($txtrecords)) ? $txtrecord = $txtrecords[0]['txt'] : $txtrecord = $domainName;
                                
                                if(!isset($reasons[$domainName])){

                                    $reasons[$domainName] = array(
                                        'domain' => $domainName,
                                        'reason' => $txtrecord
                                    );
                                    ...
                                    $message = '<pre><b>Tracking Ip Blacklisted</b> - <br/><br/>'
                                        ...
                                        . 'Blacklist Server Name - ' . $domainName . '<br/>'
                                        . 'Reason - '. $txtrecord . '<br/>'
                                        . 'Date - ' .date('Y-m-d H:i:s', strtotime('' . date('Y-m-d H:i:s') . ' +5 hours 30 minutes')). '<br/></pre>';
                                    $this->Builder_model->MailgetMail('satish.thapa.00@gmail.com','['. strval($user_data_arr_row->domain_ip) . '] - Tracking Ip Blacklisted',$message, 'MailGet');
                                }
                                            
                                                            
                            }elseif(isset($reasons[$domainName])){
                                 
                                    $delisted_message = true;
                                }
                        }elseif(isset($reasons[$domainName])){
                ...
                if($delisted_message) {
                        ...
                        . 'Blacklist IP/Domain Name - ' . $user_data_arr_row->domain_ip . '<br/>'
                        . 'Blacklist Server Name - ' . $domainName . '<br/>'
                        . 'Reason - '. $reasons[$domainName]->reason . '<br/>'
                        . 'Date - ' .date('Y-m-d H:i:s', strtotime('' . date('Y-m-d H:i:s') . ' +5 hours 30 minutes')). '<br/></pre>';
                        unset($reasons[$domainName]);
                        $this->Builder_model->MailgetMail('satish.thapa.00@gmail.com','['. $user_data_arr_row->domain_ip . '] - Tracking Ip Delisted',$message, 'MailGet');
                            ...
                            foreach ($txtrecords as $txtrecord) {
                                if(!isset($reasons[$hostname])){
                                    ...
                                    $message = '<pre><b>Tracking Ip Blacklisted</b> - <br/><br/>'
                                        ...
                                        . 'Date - ' .date('Y-m-d H:i:s', strtotime('' . date('Y-m-d H:i:s') . ' +5 hours 30 minutes')). '<br/></pre>';
                                    $this->Builder_model->MailgetMail('satish.thapa.00@gmail.com','['. $user_data_arr_row->domain_ip . '] - Tracking Ip Blacklisted',$message, 'MailGet');
                                }
                            }
